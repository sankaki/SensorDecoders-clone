...
function milesightDeviceEncode(payload) {
    ...
    if ("open_window_detection" in payload) {
        encoded = encoded.concat(setOpenWindowDetection(payload.open_window_detection.enable, payload.open_window_detection.rate, payload.open_window_detection.time));
    }
    if ("restore_open_window_detection_status" in payload) {
        encoded = encoded.concat(restoreOpenWindowDetection(payload.restore_open_window_detection_status));
    }
    ...
    if ("freeze_protection" in payload) {
        encoded = encoded.concat(setFreezeProtection(payload.freeze_protection.enable, payload.freeze_protection.temperature));
    }
